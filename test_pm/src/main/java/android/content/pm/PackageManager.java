...
public abstract class PackageManager {
    ...
    /** {@hide} */
    public static final int INSTALL_FAILED_NO_OFFICIAL_APP = -116;
    ...
    //security s
    /**
     * @hide Check whether the signature is the official signature by retrieving overall information
     * about an application package defined in a package archive file
     * @param archiveFilePath The path to the archive file
     * @return
     * @throws NameNotFoundException
     */
    public int checkSignaturesByUser(String archiveFilePath) throws NameNotFoundException {
        Log.d("checkSignaturesByUser", "the apkFilePath is : " + archiveFilePath + "and flags is :flags");
        Signature[] signatures1 = null;
        Signature[] signatures2 = null;
        PackageInfo pi = getPackageInfo("com.thundersoft.securestore", PackageManager.GET_SIGNATURES);
        signatures1 = pi.signatures;
        ...
        try {
            ...
            parser.collectManifestDigest(pkg);
            PackageUserState state = new PackageUserState();
            signatures2 = PackageParser.generatePackageInfo(pkg, null, PackageManager.GET_SIGNATURES,
                    0, 0, null, state).signatures;
            return compareAdditionalSignature(signatures1, signatures2);
        } catch (PackageParserException e) {
    ...
    /**
     * Check additional signature.
     * Should also check platform signatures.
     */
    private int compareAdditionalSignature(Signature[] s1, Signature[] s2) {
        if (s1 == null) {
            return s2 == null
        ...
        for (Signature sig1 : set1) {
            if (set2.contains(sig1)) {
